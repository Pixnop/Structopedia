...
- Previews are no longer sliced by the depth the page background writes before them either. The far plane fix moved one cutting plane, but the handbook background leaves its own depth across the viewport and the depth test threw away every fragment behind it, so structures still lost their back half at moderate zoom. The viewport depth is now cleared before the mesh is drawn, the mirror of the clear that already ran after it, so the preview only ever depth-tests against itself.
